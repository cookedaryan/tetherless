...
public class DHUtils {
    ...
    public static byte[] generateSharedSecret(PrivateKey privateKey, PublicKey otherPartyPublicKey, byte[] salt, byte[] info) throws Exception {
        ...
        try {
            ...
            // Wiped once the session key has been derived from it. This is the raw 2048-bit
            // agreement, the input every message key descends from, and it was simply dropped for
            // the collector to overwrite whenever it got round to it - which means it sat in the
            // heap, readable to a crash dump or anything able to inspect the process, long after
            // it stopped being needed.
            //
            // In the exact-256 case the two names are the same array, so the guard is not
            // tidiness: zeroing the alias twice is harmless, but the padded copy must not be
            // wiped before the value has been derived. Hence the finally rather than a wipe
            // between the two statements.
            Arrays.fill(rawSecret, (byte) 0);
            if (paddedSecret != null && paddedSecret != rawSecret) {
    ...
    public static byte[] hkdfSha256(byte[] ikm, byte[] salt, byte[] info, int length) throws Exception {
        ...
        try {
            ...
            // The extract stage's pseudorandom key, and the last expand block. Both are key
            // material in their own right - prk derives every byte of the output - and neither is
            // wanted after the caller has its key. Only okm leaves this method alive.
