...
The relay ships with a self-signed certificate generated by `scripts/generate-dev-cert.sh`, pinned
by clients. This is for local development. A deployment must provision a real certificate; the
default keystore password is the well-known `changeit`.
